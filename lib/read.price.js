...
module.exports = function(name, interval, periods){
    ...
    price_data = price_content.map( r=>{
        ...
        let percent_from_close = (d[4] / prev_close) - 1 // this close from last close
    ...
    return [
        total_range / calc.range.length, 
        total_percent / calc.percent.length, 
        total_percent_from_close / calc.percent_from_close.length
    ]
}