...
//     return 10000 * Math.pow(2, retryCount) // exponential backoff 20s -> 80s
// }}
const retryConfig = {times: 2, interval: 61000}
/**
...
module.exports = function(ticker, method, interval, key){
    return function(callback){
        ...
        series([
            ...
            function(cb){
                var method = interval==='60min' ? 'INTRADAY&interval='+interval : interval.toUpperCase()
                retry(retryConfig, function(retryCB){
                    request(uri+'TIME_SERIES_'+method+'&outputsize=full', function(err, resp, body){
                        if(err){
                            return retryCB(err, body)
                        }
                        body = body.replace(/\r\n/g, '\n')
