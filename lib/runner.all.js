/**
 * fetches tradingview technicals for all time periods (in a round-robin loop)
 */
    ...
    getSignals(config.periods[runnerData.period], (err, data)=>{
        ...
        data.forEach(t=>{
            ...
            if( isUndefined(newValue) || isNull(newValue)) {
                console.error('error getting technical signal', t)
                // don't accept it
                newValue = prevValue
            }
            ...
            tickerResults.meta = mean(tickerResults.sum)
            tickerResults.from = mean(tickerResults.prev)
