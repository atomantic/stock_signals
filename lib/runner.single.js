...
const calcHistorics = require('./calc.historics')
...
function getSignal() {
	...
	if(ticker.indexOf('crypto:')!==-1){
		...
		if(!runner.timeseriesThrottled && !afterHours){
			...
			console.log(`skip timeseries: ${runner.timeseriesThrottled?'throttled':'active'}, ${afterHours?'after hours':'in hours'}`)
		}
	}
	parallel(tasks, function(err, data){
		...
		if(data[1]){
			if(data[1].ex) tickerResults.div_ex = data[1].ex
			// if(data[1].rec) tickerResults.div_rec = data[1].rec
			if(data[1].amount) tickerResults.div_pay = data[1].amount
		}
		if(!tickerResults.historics){
			tickerResults.historics = Object.assign({}, config.tickerSchema.historics)
		}
		// add historical timeseries calculations
		// so we can do linear regression/deviation calculations
		config.historic_intervals.forEach((interval_name, i) => {
			tickerResults.historics[i] = calcHistorics(tickerName, interval_name)
		})
		...
		save(results, function(){
			...
			const currentLog = ticker+','+
				...
				tickerResults.div_ex+','+
				tickerResults.div_rec+','+
				tickerResults.div_pay+','+
