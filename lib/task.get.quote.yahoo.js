    ...
    return function(cb){
        request('https://finance.yahoo.com/quote/'+tickerName, function(err, resp, body){
            ...
            if(isNaN(price) || price===0){
                console.log(`failed to find price for ${tickerName} in yahoo, using google...`)
                ...
                cb(null, {
                    price: price, 
                    nextEarn: $('[data-test=EARNINGS_DATE-value]').text(),
                    nextDiv: $('[data-test=EX_DIVIDEND_DATE-value]').text()
                })
            }
        })
    }
