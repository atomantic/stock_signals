...
casper.test.begin('getting '+ticker, function (test) {
	...
	var getSignalValues = function () {
		...
		for(var i=0;i<oscIndexes.length;i++){
			dataObj.osc.push(Number(document.querySelector(
		...
		dataObj.osc.push(numeric[document.querySelector(
		...
		for(var i=0;i<maIndexes.length;i++){
			dataObj.ma.push(Number(document.querySelector(
				'#technicals-root > div > div > div:nth-child(3) > table:last-child > tbody > tr:nth-child('+maIndexes[i]+') > td:nth-child(2)'
			).innerHTML));
		}
