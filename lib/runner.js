...
function getSignal() {
	...
	scraper.on('close', function (code) {
		...
		scraperData.split('\n').forEach(function (l) {
			...
			if (l.includes('data-technicals_1day:')) {
				try{
					...
					if(!cache.results[ticker]){
						cache.results[ticker] = {
							day: {},
							week: {}
						}
					}
					cache.results[ticker].day = mappedData
					fs.writeFile('./data/results.json', JSON.stringify(merge({},results, cache), null, 2), function (err) {
		...
		if (error) {
			...
			console.error('scraper: ☠ ', ticker, testTime, 'seconds', errorLine)
			// log(scraperData)
		}else{
