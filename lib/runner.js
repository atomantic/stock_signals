...
var iconHelp = ''
each(config.maps.icons, (k,v) => iconHelp+=outputPad(k+': '+config.maps.reverseValues[v]+' ('+v+')', 20))
...
function getSignal() {
	...
	request('https://finance.yahoo.com/quote/'+tickerName, function(err, resp, body){
		...
		if(isNaN(price) || price===0){
			...
			fs.writeFile('./data/error-'+tickerName+'.html', body)
		...
		scraper.on('close', function (code) {
			...
			if (error) {
				...
				log(
					chalk.blue(outputPad(ticker, 15)),
					chalk.cyan(outputPad('$'+tickerResults.price.toFixed(2), 9, 'left')),
					outputPad('4H:'+colorize(tickerResults.sum[0])+' '+
					'D:'+colorize(tickerResults.sum[1])+' '+
					'W:'+colorize(tickerResults.sum[2])+' '+
					'M:'+colorize(tickerResults.sum[3]), 24),
					' '+config.maps.icons[tickerResults.meta]+' ',
					outputPad(config.maps.reverseValues[tickerResults.meta], 12),
