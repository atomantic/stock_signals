	...
	parallel(tasks, function(err, data){
		...
		if (error) {
			...
			if(data.length==3){
				...
				// console.log(daily)
				if(daily && (!isEmpty(daily.overbought[0]) || !isEmpty(daily.overbought[1]))){
					// overwrite the daily RSI
					// console.log('replace', tickerResults.overbought.state[1], daily.overbought)
					tickerResults.overbought.state[1] = daily.overbought
				}
				if(daily && (!isEmpty(daily.oversold[0]) || !isEmpty(daily.oversold[1]))){
					// console.log('replace', tickerResults.oversold.state[1], daily.oversold)
					tickerResults.oversold.state[1] = daily.oversold
				}
				const weekly = getTimeseriesData(ticker, tickerName, 'RSI', 'weekly')
				if(weekly && (!isEmpty(weekly.overbought[0]) || !isEmpty(weekly.overbought[1]))){
					// console.log('replace', tickerResults.overbought.state[2], daily.overbought)
				...
				if(monthly && (!isEmpty(monthly.overbought[0]) || !isEmpty(monthly.overbought[1]))){
					tickerResults.overbought.state[3] = monthly.overbought
				}
				if(monthly && (!isEmpty(monthly.oversold[0]) || !isEmpty(monthly.oversold[1]))){
