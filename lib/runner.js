	...
	request('https://finance.yahoo.com/quote/'+tickerName, function(err, resp, body){
		...
		scraper.on('close', function (code) {
			...
			if (error) {
				...
				log(
					outputPad(ticker, 15),
					outputPad('$'+tickerResults.price.toFixed(2), 8, 'left'),
					'4H:'+tickerResults.sum[0],
					'D:'+tickerResults.sum[1],
					'W:'+tickerResults.sum[2],
					'M:'+ tickerResults.sum[3],
					' '+config.maps.icons[tickerResults.meta]+' ',
					outputPad(config.maps.reverseValues[tickerResults.meta], 11),
