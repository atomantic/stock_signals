...
const {	cloneDeep, each, isEqual, mean, map, merge, now } = require('lodash')
...
function getSignal() {
	...
	parallel([
		...
		tickerResults.change = Number((price - previousLog.price) / previousLog.price).toFixed(4)

		// temporary to fix backward compatibility (older records)
		// delete after full run
		if(!previousLog.ma_change){
			previousLog.ma_change = []
		}
		if(!tickerResults.ma_change){
			tickerResults.ma_change = []
		}
		...
		const formatTechnicals = function(l, index){
			try{
				...
				// oscillators
				tickerResults.osc[index] = technicals.osc
				// moving averages
				tickerResults.ma[index] = technicals.ma
				// change to MA
				tickerResults.ma_change[index] = []
				for(var i=0;i<technicals.ma.length; i++){
					var ma_change = 0
					if(previousLog.ma[i]){
						ma_change = Number(((technicals.ma[i] - previousLog.ma[i]) / previousLog.ma[i]).toFixed(4))
					}
					tickerResults.ma_change[index].push(ma_change)
				}
			}catch(e){
			...
			fs.writeFile('./data/latest.json', JSON.stringify(results, null, 2), function (err) {
				...
				if(changed){
					rollingLog(
						...
						tickerResults.osc.join(',')+
						tickerResults.ma.join(',')+
						tickerResults.ma_change.join(',')
					)
				}
				// log(scraperData)
