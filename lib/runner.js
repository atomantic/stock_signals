...
const {	cloneDeep, isEqual, mean, map, merge, now } = require('lodash')
...
function getSignal() {
	...
	parallel([
		...
		// temporary cleanup of previous log state
		// for(var i=0;i<results.tickers[ticker].osc.length;i++){
		// 	for(var j=0;j<results.tickers[ticker].osc[i].length;j++){
		// 		if(results.tickers[ticker].osc[i][j] && typeof results.tickers[ticker].osc[i][j]!=="object"){
		// 			// turn into array of arrays
		// 			results.tickers[ticker].osc[i][j] = map(results.tickers[ticker].osc[i][j], n=>[n])
		// 		}
		// 	}
		// }
		// for(var i=0;i<results.tickers[ticker].ma.length;i++){
		// 	for(var j=0;j<results.tickers[ticker].ma[i].length;j++){
		// 		if(results.tickers[ticker].ma[i][j] && typeof results.tickers[ticker].ma[i][j]!=="object"){
		// 			// turn into array of arrays
		// 			results.tickers[ticker].ma[i][j] = map(results.tickers[ticker].ma[i][j], n=>[n])
		// 		}
		// 	}
		// }
		...
		const formatTechnicals = function(l, index){
			try{
				...
				// oscillators
				// correct any old format entries (temporary)
				if(tickerResults.osc[index] && tickerResults.osc[index][0] && typeof tickerResults.osc[index][0]!=="object"){
					// turn into array of arrays
					tickerResults.osc[index] = map(tickerResults.osc[index], n=>[n])
				}
								// now push each item onto the array
				// (only if the value is different from the previous value)
				// this allows us to calculate directionality (and last state)
				// as well as create column/line sparklines in the sheet
				for(var i=0; i<technicals.osc.length; i++){
					if(!tickerResults.osc[index][i]){
						tickerResults.osc[index][i] = [technicals.osc[i]]
					}else if(tickerResults.osc[index][i][tickerResults.osc[index][i].length-1]!==technicals.osc[i]){
						tickerResults.osc[index][i].push(technicals.osc[i])
					}
					// only keep 2 most recent values
					if(tickerResults.osc[index][i].length > 2){
						tickerResults.osc[index][i].shift()
					}
				}
				// moving averages
				// correct any old format entries (temporary)
				if(tickerResults.ma[index] && typeof tickerResults.ma[index][0]!=="object"){
					// turn into array of arrays
					tickerResults.ma[index] = map(tickerResults.ma[index], n=>[n])
				}
				for(var i=0; i<technicals.ma.length; i++){
					if(!tickerResults.ma[index][i]){
						tickerResults.ma[index][i] = [technicals.ma[i]]
					}else if(tickerResults.ma[index][i][tickerResults.ma[index][i].length-1]!==technicals.ma[i]){
						tickerResults.ma[index][i].push(technicals.ma[i])
					}
					// only keep 2 most recent values
					if(tickerResults.ma[index][i].length > 2){
						tickerResults.ma[index][i].shift()
					}
				}
			}catch(e){
