...
const {	cloneDeep, isEmpty, isEqual, isNull, isUndefined, mean, merge, now } = require('lodash')
...
const getTimeseriesData = require('./get.timeseries.data')
...
function getSignal() {
	...
	const tickerSplit = ticker.replace('crypto:', '').split('-')
	...
	parallel(tasks, function(err, data){
		...
		if (error) {
			...
			if(data.length==3){
				// timeseries data from AlphaVantage API
				const hourly = getTimeseriesData(ticker, tickerName, 'RSI', '60min')
				// console.log(daily)
				if(hourly && 
					(
						(!isEmpty(hourly.overbought[0]) || !isEmpty(hourly.overbought[1])) ||
						(!isEmpty(hourly.oversold[0]) || !isEmpty(hourly.oversold[1]))
					)
				){
					tickerResults.overbought.state[0] = hourly.overbought
					tickerResults.oversold.state[0] = hourly.oversold
					if(
						!isEmpty(tickerResults.overbought.state[0][0]) && 
						// previous price is > latest price
						tickerResults.overbought.state[0][0][1] > tickerResults.overbought.state[0][1][1]
					){
						console.log('hourly bearish divergence')
						tickerResults.divergence[0] = -1
					}else if(
						!isEmpty(tickerResults.oversold.state[0][0]) && 
						tickerResults.overbought.state[0][0][1] < tickerResults.overbought.state[0][1][1]
					){
						console.log('hourly bullish divergence')
						tickerResults.divergence[0] = 1
					}else{
						tickerResults.divergence[0] = 0
					}

				}
				// timeseries data from AlphaVantage API
				const daily = getTimeseriesData(ticker, tickerName, 'RSI', 'daily')
				// console.log(daily)
				if(daily && 
					(
						(!isEmpty(daily.overbought[0]) || !isEmpty(daily.overbought[1])) ||
						(!isEmpty(daily.oversold[0]) || !isEmpty(daily.oversold[1]))
					)
				){
					// overwrite the daily RSI collected from tradingview (Alphavantage is more accurate)
					tickerResults.overbought.state[1] = daily.overbought
					tickerResults.oversold.state[1] = daily.oversold
					if(
						!isEmpty(tickerResults.overbought.state[1][0]) && 
						// previous price is > latest price
						tickerResults.overbought.state[1][0][1] > tickerResults.overbought.state[1][1][1]
					){
						console.log('daily bearish divergence')
						tickerResults.divergence[1] = -1
					}else if(
						!isEmpty(tickerResults.oversold.state[1][0]) && 
						tickerResults.overbought.state[1][0][1] < tickerResults.overbought.state[1][1][1]
					){
						console.log('daily bullish divergence')
						tickerResults.divergence[1] = 1
					}else{
						tickerResults.divergence[1] = 0
					}

				}
				const weekly = getTimeseriesData(ticker, tickerName, 'RSI', 'weekly')
				if(weekly && 
					(
						(!isEmpty(weekly.overbought[0]) || !isEmpty(weekly.overbought[1])) ||
						(!isEmpty(weekly.oversold[0]) || !isEmpty(weekly.oversold[1]))
					)
				){
					tickerResults.overbought.state[2] = weekly.overbought
					tickerResults.oversold.state[2] = weekly.oversold
					if(
						!isEmpty(tickerResults.overbought.state[2][0]) && 
						// previous price is > latest price
						tickerResults.overbought.state[2][0][1] > tickerResults.overbought.state[2][1][1]
					){
						console.log('weekly bearish divergence')
						tickerResults.divergence[2] = -1
					}else if(
						!isEmpty(tickerResults.oversold.state[2][0]) && 
						tickerResults.overbought.state[2][0][1] < tickerResults.overbought.state[2][1][1]
					){
						console.log('weekly bullish divergence')
						tickerResults.divergence[2] = 1
					}else{
						tickerResults.divergence[2] = 0
					}
				}
				if(weekly && (!isEmpty(weekly.oversold[0]) || !isEmpty(weekly.oversold[1]))){
					// console.log('replace', tickerResults.oversold.state[2], daily.oversold)
					tickerResults.oversold.state[2] = weekly.oversold
				}
				const monthly = getTimeseriesData(ticker, tickerName, 'RSI', 'monthly')
				if(monthly && 
					(
						(!isEmpty(monthly.overbought[0]) || !isEmpty(monthly.overbought[1])) ||
						(!isEmpty(monthly.oversold[0]) || !isEmpty(monthly.oversold[1]))
					)
				){
					tickerResults.overbought.state[3] = monthly.overbought
					tickerResults.oversold.state[3] = monthly.oversold
					if(
						!isEmpty(tickerResults.overbought.state[3][0]) && 
						tickerResults.overbought.state[3][0][1] > tickerResults.overbought.state[3][1][1]
					){
						console.log('monthly bearish divergence')
						tickerResults.divergence[3] = -1
					}else if(
						!isEmpty(tickerResults.oversold.state[3][0]) && 
						tickerResults.overbought.state[3][0][1] < tickerResults.overbought.state[3][1][1]
					){
						console.log('monthly bullish divergence')
						tickerResults.divergence[3] = 1
					}else{
						tickerResults.divergence[3] = 0
					}
				}
			}else{
