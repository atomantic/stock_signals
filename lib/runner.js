	...
	parallel(tasks, function(err, data){
		...
		if (error) {
			...
			if(data.length==3){
				...
				const daily = getTimeseriesData(tickerName, 'RSI', 'daily')
				// console.log(daily)
				if(!isEmpty(daily.overbought[0]) || !isEmpty(daily.overbought[1])){
