...
const {	cloneDeep, each, mean, merge, now } = require('lodash')
...
function getSignal() {
	...
	// this is stupid but TradingView isn't rendering properly in casper/phantomjs, so there is no price rendered
	// instead, we have to hit the yahoo finance API, but it uses a funky way of specifying the market
	// so anything on AMEX- (e.g. AMEX-REMX) needs to be specified with a market extension
	// however, I can't find the proper extension for REMX... but I can simply load the page as "REMX".
	// works:
	...
	request('https://finance.yahoo.com/quote/'+tickerName, function(err, resp, body){
		...
		tickerResults.price = Number(price)
	})
	...
	scraper.on('close', function (code) {
		...
		if (error) {
			...
			log(
				outputPad(ticker, 15),
				tickerResults.price,
				config.maps.changeIcons[Math.round(tickerResults.meta - tickerResults.from)],
