...
function getSignal() {
	...
	scraper.on('close', function (code) {
		...
		let previousLog = results.results[ticker]
		// the last runs meta signal
		previousSignal = previousLog && previousLog.signal ? previousLog.signal : 'Neutral'
		// the last runs change from signal
		// we will forward this along if the current signal is the same as the last run signal
		// so we will always show this as changed from the previous state
		previousFrom = previousLog && previousLog.from ? previousLog.from : 'Neutral'
		...
		if (error) {
			...
			// set the new "from" value to either the previous from (if nothing changed)
			// or to the last signal that we changed away from
			tickerResults.from = tickerResults.signal===previousSignal ? previousFrom : previousSignal
			...
			log(
				outputPad(ticker, 15),
				changeIcons[Math.round(stateValues[tickerResults.signal] - stateValues[tickerResults.from])],
			...
			rollingLog(ticker+'\t'+tickerResults.updated+'\t'+tickerResults.signal)
			// log(scraperData)
		}
		setTimeout(function () {
			getNextSignal()
