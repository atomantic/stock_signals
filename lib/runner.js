...
function getSignal() {
	...
	scraper.on('close', function (code) {
		...
		if (error) {
			...
			var icon = '✅ '
			if(mappedData.Summary=='Strong Buy' || mappedData.Summary=='Buy'){
				icon = '📈 '
			}
			if(mappedData.Summary=='Strong Sell' || mappedData.Summary=='Sell'){
				icon = '📉 '
			}
			log(icon, outputPad(ticker, 12), mappedData.Summary)
		}
