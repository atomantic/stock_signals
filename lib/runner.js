...
function getSignal() {
	...
	if(ticker.includes('swap:')){
		...
		// these are meant to be swapped regularly (e.g. SOXS/SOXL)
		const timeseriesParams = getTimeseriesParams(ticker, 'swap')
		const timeseriesTask = taskGetTimeseries(tickerName, timeseriesParams.method, timeseriesParams.interval)
		timeseriesTask(function(){
			...
			log(ticker, tickerResults.rsi, tickerResults.cci, now() - runner.start)
		})
		return
	}
